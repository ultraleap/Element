...
namespace Element.AST
{
    ...
    public static class ValueExtensions
    {
        ...
        public static Result<List<IValue>> EnumerateValues(this IDeclarationScope declarationScope, CompilationContext context, Predicate<Declaration> declarationFilter = null, Predicate<IValue> resolvedValueFilter = null)
        {
            ...
            void Recurse(IScope containingScope, IEnumerable<Declaration> declarations)
            {
                foreach (var decl in declarations)
                {
                    if (declarationFilter?.Invoke(decl) ?? true)
                    {
                        ...
                        builder.Append(decl.Resolve(containingScope, context)
                                           .Do(AddResolvedValueToResults));
                    }

                    if (!(decl.Body is Block block)) continue;
                    builder.Append(block.Resolve(containingScope, context)
                                        .Do(resolvedBlockScope => Recurse(resolvedBlockScope, block)));
                }
            }
            Recurse(declarationScope, declarationScope);

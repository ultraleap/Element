...
namespace Element.NET.Tests
{
    public class EnumerateValues : FixtureBase
    {
        ...
        public void EnumerateAll()
        {
            ...
            var results = sourceContext.GlobalScope.EnumerateValues(Context.CreateFromSourceContext(sourceContext));
        ...
        {
            var sourceContext = MakeSourceContext();
            var results = sourceContext.GlobalScope.EnumerateValues(Context.CreateFromSourceContext(sourceContext), d => d.Identifier.String.Contains(nameContains, StringComparison.OrdinalIgnoreCase));
            Assert.That(results.IsSuccess);
            CollectionAssert.IsNotEmpty(results.ResultOr(default));
        ...
        {
            var sourceContext = MakeSourceContext();
            bool Filter(ValueWithLocation v) => v.WrappedValue.ReturnConstraint.IsSpecificIntrinsic(intrinsic);
            var results = sourceContext.GlobalScope.EnumerateValues(Context.CreateFromSourceContext(sourceContext), resolvedValueFilter: Filter);
            Assert.That(results.IsSuccess);
            CollectionAssert.IsNotEmpty(results.ResultOr(default));
            // TODO: Actually check collection contents are correct
        }

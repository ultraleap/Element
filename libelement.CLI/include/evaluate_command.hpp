...
namespace libelement::cli
{
	...
	public:
		...
		[[nodiscard]] compiler_message execute(const compilation_input& compilation_input) const override
		{
			...
			result = element_interpreter_evaluate_expression(context, nullptr, custom_arguments.expression.c_str(), &output);
			if (result != ELEMENT_OK) {
				return compiler_message(error_conversion(result), "Failed to evaluate: " + custom_arguments.expression + " with element_result " + std::to_string(result));
			}

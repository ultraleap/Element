...
{
    auto expr = std::dynamic_pointer_cast<const instruction_constant>(compiled_args[0]);
    if (!expr) {
    ...
    return evaluate(context, context.interpreter->cache_instruction_constant.get(expr->value(), type::num.get()));
}

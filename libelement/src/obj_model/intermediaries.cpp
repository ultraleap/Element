...
//compiled_expression
std::shared_ptr<element::element_object> element::compiled_expression::index(const compilation_context& context, const identifier& identifier) const
{
    if (object)
        return object->index(context, identifier);

    //todo: is there any point?
    if (declarer)
    {
        assert(false); //figure out if/when this occurs and if it should
        return declarer->index(context, identifier);
    }

    return nullptr;
}

std::shared_ptr<element::element_object> element::compiled_expression::call(const compilation_context& context, std::vector<std::shared_ptr<compiled_expression>> args) const
{
    if (object)
        return object->call(context, std::move(args));

    //todo: is there any point?
    if (declarer)
    {
        assert(false); //figure out if/when this occurs and if it should
        return declarer->call(context, std::move(args));
    }
...
std::shared_ptr<element::element_object> element::struct_instance::index(const compilation_context& context, const identifier& identifier) const
{
...
std::shared_ptr<element::element_object> element::function_instance::call(const compilation_context& context, std::vector<std::shared_ptr<compiled_expression>> args) const
{
